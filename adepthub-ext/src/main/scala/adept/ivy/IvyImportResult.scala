...
case class IvyImportResultMetadata(id: Id, variant: VariantMetadata, artifacts: Set[Artifact], repository: RepositoryName, versionInfo: Set[VersionInfo], excludeRules: Set[AdeptExcludeMapping], extendsIds: Set[Id])
...
object IvyImportResultsMetadata {
  ...
  private[adept] implicit val ivyImportResultFormatArtifact: Format[Artifact] = {
    (
      (__ \ "hash").format[String] and
      (__ \ "size").format[Long] and
      (__ \ "locations").format[Set[String]])({
        case (hash, size, locations) =>
          new Artifact(new ArtifactHash(hash), size, locations.map(new ArtifactLocation(_)).asJava)
      }, unlift({ a: Artifact =>
        Some((a.hash.value, a.size, Set() ++ a.locations.asScala.map(_.value)))
      }))
  }
