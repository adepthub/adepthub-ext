    ...
    grouped.par.foreach { //NOTICE .par TODO: replace with something more optimized for IO not for CPU
      case (_, results) =>
        results.foreach { result =>
          ...
          val existingVariantMetadata = VariantMetadata.read(id, variantMetadata.hash, repository, repository.getHead, checkHash = true)
          if (!existingVariantMetadata.isDefined) { //this variant exists already so skip it
            ...
            val (addFiles, rmFiles) = useDefaultVersionRanking(id, variantMetadata.hash, repository, commit)
