...
object AdeptUIService {
  ...
  val applySemVerURI = "semver"
  val ApplySemVerURIURIRegEx = (applySemVerURI + "/" + """(.*)""" +"/" + """(.*)""").r
...
class AdeptUIService extends Actor {
  ...
  def receive = {
    ...
    case HttpRequest(GET, Uri.Path(path), _, _, _) if ApplySemVerURIURIRegEx.findAllMatchIn(path).nonEmpty =>
      path match {
